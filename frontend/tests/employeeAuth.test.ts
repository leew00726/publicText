...
describe('employee auth helpers', () => {
  ...
  it('maps module access by employee role', () => {
    ...
    expect(staffModules.find((item) => item.key === 'meetingMinutes')?.entryPath).toBe('/meeting-minutes')
    expect(staffModules.find((item) => item.key === 'management')?.enabled).toBe(false)
    expect(staffModules.find((item) => item.key === 'meetingMinutes')?.enabled).toBe(true)
    expect(adminModules.find((item) => item.key === 'management')?.enabled).toBe(true)
    expect(adminModules.find((item) => item.key === 'meetingMinutes')?.enabled).toBe(true)
  })
})
