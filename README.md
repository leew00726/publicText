...
本轮优先完成排版、模板、编辑、校验、导入导出及业务交互整改；登录、认证和权限问题按当前安排暂缓。

### 已完成

- 统一模板规则 V1：新增 `schemaVersion`，训练、草稿、正式模板和文档快照统一归一化；历史 `pageMargins/topCm` 等旧字段可自动迁移。
- 打通排版一致性：模板训练得到的上、下、左、右页边距由前端 A4 预览和后端 DOCX 导出共同读取，不再使用导出端固定边距。
- 完善模板发布与版本管理：同一草稿重复确认不再生成重复版本；支持查看版本规则、切换当前生效版本和删除版本。
- 增加样本冲突策略：可选择保留当前规则并复核、优先文字规则或优先样本规则，并展示冲突数量与规则路径。
- 完善编辑器保存状态：正文编辑支持 1.5 秒自动保存、手动保存、未保存离开提醒，以及“未保存 / 保存中 / 已保存 / 保存失败”状态反馈。
- 完善校验链路：校验前强制保存最新内容；区分尚未校验、校验中、内容已修改和校验结果；空正文、空主标题和显式模板样式偏差均会提示。
- 增强一键排版：支持撤销最近一次自动排版，降低批量格式修正的操作风险。
- 改进 DOCX 导入导出：导入前明确提示会替换正文，替换后保留当前文档 ID、题材和模板版本快照；导出前自动校验并展示进度与完成反馈。
- 区分文档名称和公文主标题，明确列表/文件名字段与正文显示标题的用途。
- 公文管理仅保留公司、题材和模板治理，已删除其中的文档库功能及相关入口；公文排版流程中的题材文档库继续保留。
- 公司和题材列表增加搜索与结果数量；删除类操作统一使用危险操作视觉层级。
- 上传、导入和导出的文件存储增加失败补偿，降低数据库与文件状态不一致的风险。
- 优化工作台大屏空间利用：工作区自适应覆盖剩余视口，模块仅适度延展并在可用区域内均衡排列；窄屏继续使用自然高度，避免强制拉伸。
- 删除公文总结的“总结长度”设置及前后端字数限制参数，生成结果改为按原文内容、默认结构和用户补充要求自然组织。
- 公文总结输入控制台改为按桌面可用高度自适应分配上传区、知识库区和补充要求区，主操作按钮自然贴近面板底部；在常用桌面和超宽大屏视口下，默认上传与粘贴输入均可完整填充且无需内部滚动条。
...
- 后端自动化测试：88 项通过。
- 前端生产构建：成功。
- 浏览器关键链路验收：模板训练、模板选择、正文编辑、自动保存、空正文校验和模板快照提示均通过；关键页面未发现横向布局溢出。
- 已知非阻断提示：前端主包约 660 KB，React Router 存在未来版本兼容提示，Python 存在 `datetime.utcnow()` 弃用提示。

### 暂缓与后续

- 按当前安排暂缓：登录认证、JWT/Session、租户权限隔离、默认密码和管理员范围治理。
- 数据模型已声明草稿来源唯一约束，现有数据库仍需通过正式迁移工具落库。
- 后台任务、列表分页、前端代码分包和上述依赖升级不属于本轮功能链路整改，后续按性能与运维优先级推进。
...
- 草稿版本：每次分析/修订生成新草稿版本。
- 模板确认：草稿确认后形成正式模板版本；重复确认保持幂等，并可切换生效版本。
- 冲突处理：样本规则与文字规则冲突时，可保留并复核或明确选择优先来源。
...
- 自动保存：编辑后自动保存并反馈保存状态，校验和导出始终使用最新正文。
- 导入 DOCX：抽取正文结构并输出导入报告；替换当前正文时保留题材、模板版本及规则快照。
- 导出 DOCX：保持红头、页码、边距、标题层级、尾部格式一致，并在导出前执行规范校验。
...
### 3.6 规范与合规能力
- 规范校验：空内容、节点类型、标题层级、编号连续性、标题句末标点、正文缩进，以及字体、字号、行距、对齐与模板的偏差。
- 一键排版：自动规范标题编号/标点、正文段落属性、附件格式等，并支持撤销最近一次排版。
...
- `POST /api/topics/{topic_id}/templates/{template_id}/activate`
...
- 训练推断、规则迁移、版本管理、正文自动保存、模板一致性校验、导入导出、智能润色。
- 公文管理聚焦公司、题材和模板治理；文档库仅保留在公文排版业务流程中。

后续规划：
- 多角色协作与审批流。
- 更细粒度模板对比与回滚。
- 规则异常自动诊断与修复建议。
- 登录认证、租户权限域隔离和管理员范围治理（当前按安排暂缓）。
- 数据库正式迁移、后台任务、分页和前端代码分包。
