...
## 12. 开发进度同步（2026-06-04）

本轮开发重点围绕“可控登录权限、私有化 DeepSeek、模板排版一致性、前端体验升级”收口，当前进度如下：

- 登录与权限：登录结果已绑定后端公司记录，并返回角色权限列表；前端路由和操作入口按权限与所属公司做拦截。
- 准入白名单：已新增 `贵诚信托-葛元利-82054327`，默认密码仍沿用 `000000`。
- 模板识别：DeepSeek 可参与段落语义角色识别，补充主送、附件、落款、引用等内容角色；本地样式抽取和 DOCX 导出仍保持确定性。
- 私有化 AI：支持指向本地 OpenAI 兼容 DeepSeek 服务，`hybrid` 模式下 AI 异常会回退到本地规则，降低训练中断风险。
- 一键排版修复：导入 DOCX 时会保留题材模板上下文，导出时模板标题样式优先于导入文档残留样式，避免二级/三级标题被错误统一成黑体。
- 前端体验：整体视觉已改为 Apple-like 浅色风格，弱化模块卡片堆叠，工作台改为 grouped surface，编辑页只调整外层 chrome，不影响正文纸张预览。
- 验证情况：后端 `pytest`、前端 `npm test` 和 `npm run build` 均已通过；Vite 仍提示主 JS chunk 超过 500KB，这是既有体积提醒。

配套开发记录：
- [公司绑定登录与权限实现计划](docs/plans/2026-06-04-company-bound-auth-permissions.md)
- [DeepSeek 模板识别升级计划](docs/plans/2026-06-04-deepseek-template-inference-upgrade.md)

## 13. 当前边界与后续规划

当前已实现：
- 面向纪要/通知/请示/函的模板化排版闭环。
- 公司绑定登录、角色权限、训练推断、版本管理、正文编辑、校验、导入导出、智能润色。
