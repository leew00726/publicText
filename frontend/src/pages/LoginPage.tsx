...
export function LoginPage() {
  ...
  useEffect(() => {
    ...
    const frameId = requestAnimationFrame(() => {
      timeoutId = window.setTimeout(() => setIntroVisible(true), 240)
    })
  ...
  return (
    <main className="auth-shell">
      <section className={`auth-scene ${introVisible ? 'auth-scene-revealed' : ''}`}>
        <div className="auth-title-stage">
          <div className="auth-title-row">
            <img className="auth-title-logo" src="/huaneng-logo.jpg" alt="华能标志" />
            <h1 className="auth-title auth-title-simple">云矩公文管理平台</h1>
          </div>
        </div>
