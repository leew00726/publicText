...
describe('employee auth helpers', () => {
  ...
  it('maps module access by employee role', () => {
    ...
    expect(staffModules).toHaveLength(4)
    expect(adminModules).toHaveLength(4)
    expect(staffModules.find((item) => item.key === 'summary')?.entryPath).toBe('/summary')
    expect(staffModules.find((item) => item.key === 'knowledge')?.entryPath).toBe('/knowledge')
    expect(staffModules.find((item) => item.key === 'knowledge')?.enabled).toBe(true)
    expect(staffModules.find((item) => item.key === 'layout')?.entryPath).toBe(LAYOUT_HOME_PATH)
    expect(staffModules.find((item) => item.key === 'management')?.entryPath).toBe('/management/companies')
    expect(staffModules.find((item) => item.key === 'management')?.enabled).toBe(false)
    expect(staffModules.find((item) => item.key === 'meetingMinutes')).toBeUndefined()
    expect(adminModules.find((item) => item.key === 'management')?.enabled).toBe(true)
    expect(adminModules.find((item) => item.key === 'meetingMinutes')).toBeUndefined()
  })
})
